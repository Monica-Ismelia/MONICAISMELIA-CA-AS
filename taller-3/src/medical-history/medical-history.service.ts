...
export class MedicalHistoryService {
    ...
    async create(createDto: CreateMedicalHistoryDto): Promise<MedicalHistory>{
        const { Id_users, ...hitoryData} = createDto;

        const users = await this.userRepository.findByIds(Id_users);

        const newHistory = this.historyRepository.create({
            ...hitoryData, users: users,
        });
    ...
    async findOne(id: number): Promise<MedicalHistory>{
        ...
        if (!history){
            throw new NotFoundException(`Historial medico con el Id ${id} no fue encontrado`);
        }
        return history;
    }
    ...
    async update(id: number, updateDto: UpdateMedicalHistoryDto): Promise<MedicalHistory>{
        ...
        if(Id_users && Id_users.length > 0){
            history.users = await this.userRepository.findByIds(Id_users);
        }
